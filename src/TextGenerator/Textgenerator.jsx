...
const TextGenerator = () => {
    ...
    <div className="container shadow p-4 rounded-4 col-lg-6 col-md-8 col-sm-10 mx-auto my-4 bg-light">
        <h3 className='display-5 text-center text-primary mb-4'>Text Generator</h3>
        <form className='d-flex flex-column gap-3'>
            <label className="fw-semibold">Enter Number of text you want to generate</label>
            <input 
                value={inp} 
            ...
            />
            {err && <p className='text-danger fw-semibold'>Enter valid Number between 0 and {db.length}</p>}
    ...
    <div className="container my-4 d-flex flex-column gap-3">
        {data?.map((items,index)=>{
            return (
                <div key={index} className="shadow rounded-3 p-3 bg-white border-start border-3 border-primary">
